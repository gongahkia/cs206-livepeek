...
class TranslationService {
  ...
  // Clean up noisy English translations coming from free APIs
  sanitizeEnglish(text) {
    if (!text) return text;
    let cleaned = String(text);
    // Remove bracketed/parenthetical glosses and readings
    cleaned = cleaned.replace(/\[[^\]]*\]/g, '').replace(/\([^)]*\)/g, '');
    // Take the first sensible option when slash-delimited
    const firstOption = cleaned
      .split(/[\/|]/)
      .map(s => s.trim())
      .filter(Boolean)[0] || cleaned;
    // Collapse whitespace and trim punctuation
    cleaned = firstOption.replace(/\s+/g, ' ').trim();
  ...
  async translateText(text, fromLang = 'en', toLang = 'ja') {
    ...
    try {
      ...
      // 1. Try Google Translate (best quality)
      let translation = await this.tryGoogleTranslate(text, fromLang, toLang);
      
      // 2. Try Lingva (Google Translate proxy)
      if (!translation) {
        translation = await this.tryLingvaTranslate(text, fromLang, toLang);
      }
      ...
      // 5. Fallback to basic word-by-word translation
      if (!translation) {
        translation = await this.basicWordTranslation(text, fromLang, toLang);
      }

      // Final cleanup for English outputs from public APIs
      if (translation && toLang === 'en') {
        translation = this.sanitizeEnglish(translation);
      }
  ...
  // Google Translate API (unofficial, but best quality)
  async tryGoogleTranslate(text, fromLang, toLang) {
    try {
      // Use Google Translate's client API endpoint
      ...
      if (data && data[0] && Array.isArray(data[0])) {
        ...
        if (translation && translation !== text && translation.length > 0) {
          console.log('✅ Google Translate success:', text.substring(0, 50) + '...');
          return translation;
        }
      }
