...
import { ArrowLeft, BookOpen, Star, Trash2, ChevronDown, ChevronUp, Volume2, VolumeX } from 'lucide-react';
...
const Dictionary = ({ onBack, userDictionary, onRemoveWord }) => {
  ...
  const speakJapanese = async (word, wordId) => {
    if (!speechSupported) {
    ...
    try {
      ...
      await speechService.speakJapanese(word.japanese, {
  ...
  const speakHiragana = async (word, wordId) => {
    if (!speechSupported) {
      console.warn('Text-to-speech not supported');
      return;
    }
    ...
    try {
      setSpeakingWord(wordId);
      await speechService.speakJapanese(word.hiragana, {
        rate: 0.6, // Even slower for pronunciation practice
        pitch: 1.0,
        volume: 1.0
      });
    } catch (error) {
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-4">
                    <div className="flex items-center space-x-2">
                      ...
                      {speechSupported && (
                        <button
                          onClick={() => speakJapanese(word, word.id)}
                          className={`p-1 rounded-full transition-colors ${
                            speakingWord === word.id 
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      {speechSupported && word.hiragana !== word.japanese && (
                        <button
                          onClick={() => speakHiragana(word, `${word.id}-hiragana`)}
                          className={`p-1 rounded-full transition-colors ${
                            speakingWord === `${word.id}-hiragana` 
                              ? 'bg-green-500 text-white' 
                              : 'bg-gray-100 text-gray-600 hover:bg-green-100 hover:text-green-600'
                          }`}
                          title="Listen to hiragana pronunciation"
                        >
                          {speakingWord === `${word.id}-hiragana` ? (
